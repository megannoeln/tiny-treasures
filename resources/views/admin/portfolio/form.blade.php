...
@section('content')
    <div class="flex items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.portfolio.index') }}" class="text-sm text-zinc-300 hover:text-[#5a3f66]">Back</a>
    </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea id="description" name="description" rows="6"
                    class="mt-1 w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm outline-none ring-amber-400 focus:ring-2">{{ old('description', $artwork->description) }}</textarea>
                @error('description')<p class="mt-1 text-sm text-red-300">{{ $message }}</p>@enderror
            </div>

            <div class="flex items-end">
                <label class="flex items-center gap-2 text-sm text-zinc-300">
                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', (bool) $artwork->is_featured) ? 'checked' : '' }}
